CORE.create_module('search-box', function(sb) {
  ...
  return {
    init: function() {
      // change query method to find in sandbox
  ...
  return {
    init: function() {
      ...
      sb.listen({
        ...
        'reset-flter': this.reset,
        'do-search': this.search,
    ...
    destroy: function() {
      ...
      sb.ignore(['change-filter', 'reset-filter', 'do-filter', 'quit-search'])
    },
    reset: reset,
...
CORE.create_module('shopping-cart', function(sb) {
  ...
  return {
    ...
    addItem: function(product) {
      var entry = sb.find('#cart-' + product.id + '.quantity');
      if (entry) {
        entry.innerHTML = parseInt(entry.innerHTML, 10) + 1;
        cartItems[product.id]++;
      } else {
        entry = sb.create_element('li', { cart-product.id, children : [
